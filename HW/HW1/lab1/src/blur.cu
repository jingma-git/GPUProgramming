...
void cuda_blur_kernel_convolution(uint thread_index, const float* gpu_raw_data,
	...
	if (thread_index < blur_v_size) { // here is where warp divergence may occur
...
float cuda_call_blur_kernel(const unsigned int blocks,
    ...
    float* gpu_raw_data;
	cudaMalloc(&gpu_raw_data, sizeof(float) * n_frames);
    ...
    //       allocated.
    float* gpu_blur_v;
	cudaMalloc(&gpu_blur_v, sizeof(float) * blur_v_size);
    ...
    float* gpu_out_data;
	cudaMalloc(&gpu_out_data, sizeof(float) * n_frames);
